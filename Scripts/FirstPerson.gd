...
func _fixed_process(delta):
	...
	if (is_on_ground):
		...
		if (on_top_of.is_in_group("nextlevel")):
			get_node(global).setScene("res://Level 1.tscn")
		if (on_top_of.is_in_group("level2")):
			get_node(global).setScene("res://Level 2.tscn")
		#print("hey, it works!")
	#Player movement
	...
	if ray.is_colliding():
		...
		var visible = true
		#cursor.show()
		if (object.is_in_group("interactable") and Input.is_action_pressed("player_interact")):
			object.call("on_detected")
			print("interactable")
